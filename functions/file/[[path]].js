...
export async function onRequest(context) {  // Contents of context object
    ...
    /* CNB 渠道(公开仓直链 302 重定向) */
    if (imgRecord.metadata?.Channel === 'CNB') {
        const cnbUrl = imgRecord.metadata?.CnbUrl;
        if (!cnbUrl) {
            return new Response('Error: CNB file URL not found', { status: 500 });
        }
        if (!context.imageTransform.requested) {
            return Response.redirect(cnbUrl, 302);
        }
        try {
            ...
            return new Response(`Error: Failed to fetch CNB image - ${error.message}`, { status: 500 });
        }
    }
