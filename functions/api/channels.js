...
export async function onRequest(context) {
    ...
    try {
        ...
        // CNB 渠道:优先网页后台配置的渠道,环境变量作兜底
        const cnbChannels = (uploadConfig.cnb?.channels || []).map(ch => ({
            name: ch.name,
            type: 'CNB'
        }));
        if (cnbChannels.length > 0) {
            channels.cnb = cnbChannels;
        } else if (env.CNB_TOKEN && env.CNB_REPO) {
            channels.cnb = [{ name: 'CNB_env', type: 'CNB' }];
        }
