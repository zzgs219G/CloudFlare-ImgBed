...
async function uploadFileToCnb(context, fullId, metadata, returnLink) {
    ...
    // 优先使用网页后台配置的 CNB 渠道
    const cnbSettings = uploadConfig.cnb;
    let cnbChannel;
    if (cnbSettings && cnbSettings.channels && cnbSettings.channels.length > 0) {
        if (specifiedChannelName) {
            cnbChannel = cnbSettings.channels.find(ch => ch.name === specifiedChannelName);
        }
        if (!cnbChannel) {
            cnbChannel = cnbSettings.channels[0];
        }
    }

    // 后台未配置时,回退到环境变量(兼容)
    const cnbToken = cnbChannel?.token || env.CNB_TOKEN;
    const cnbRepo = cnbChannel?.repoUrl || env.CNB_REPO;
    if (!cnbToken || !cnbRepo) {
        return createResponse('Error: No CNB channel provided', { status: 400 });
    }
    ...
    try {
        ...
        metadata.ChannelName = cnbChannel?.name || "CNB_env";
        metadata.CnbFilePath = result.path;
        metadata.CnbUrl = result.url;
